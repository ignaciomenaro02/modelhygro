...
Optionally, `SolarCalculator(weather=...)` takes measured direct-normal and
diffuse irradiance plus the sun position from an RE2020 weather file
(weather.load_re2020_weather) instead of steps 1-2.
...
class SolarCalculator:
    ...
    weather        : Weather   Optional.  If it carries measured solar data
                           (dirN, diff, sun_alt_deg, sun_az_deg — RE2020 files,
                           hourly from 1 January), those replace the clear-sky
                           model; latitude/longitude/cloud_factor are then unused.
                           Each hourly value is held constant over its hour.
    """
    ...
    def __init__(
        ...
        self._wx = weather if getattr(weather, 'sun_alt_deg', None) is not None else None

    def _hour_index(self, t_sim: float) -> int:
        """Index of the hourly weather row containing t_sim [s from sim start]."""
        t_abs = (self.start_doy - 1) * 86400.0 + t_sim
        return int(t_abs // 3600.0) % len(self._wx.dirN)
    ...
    def position(self, t_sim: float):
        ...
        if self._wx is not None:
            i = self._hour_index(t_sim)
            return (float(np.radians(self._wx.sun_alt_deg[i])),
                    float(np.radians(self._wx.sun_az_deg[i])))
        ...
        if omega < 0:       # morning → east of south → negative in +West convention
            azimuth = -azimuth
    ...
    def horizontal_irradiance(self, t_sim: float):
        ...
        if self._wx is not None:
            i = self._hour_index(t_sim)
            Ib_h = self._wx.dirN[i] * np.sin(alt) if alt > 0 else 0.0
            return float(Ib_h), float(self._wx.diff[i])
    ...
    def irradiance(
        ...
        # Night: no sun and no diffuse (measured files still carry a little
        # diffuse light at dawn/dusk, when the altitude is clipped to 0).
        if alt <= 0 and Id_h <= 0.0:
            return 0.0
        ...
        # Direct normal irradiance: measured (RE2020 file) or air-mass model
        if self._wx is not None:
            DNI = self._wx.dirN[self._hour_index(t_sim)]
        elif alt > 0:
            doy = self._doy(t_sim)
            I0  = self._extra_terrestrial(doy)
            am  = self._air_mass(alt)
        ...
        Ib_surf = DNI * max(float(cos_theta), 0.0) if alt > 0 else 0.0

...
Météo      : fichiers RE2020 par zone (H1a…H3) — voir ZONE / MODE dans la section 2
             (python simulation_chanvre.py H3 [Th-D|Th-BC] ; "MACON" = CSV prospectif +2 °C)

Graphiques produits  (suffixe _<ZONE> ajouté à chaque nom, sauf modele_3d.pdf)
-------------------
  modele_3d.pdf          — vue 3D de la pièce (4 parois, fenêtres, cotes)
  serie_temp_HR.pdf      — séries temporelles T et HR pièce vs extérieur
...
# Mode : "Th-D" = confort d'été (DH)  |  "Th-BC" = besoins/consommations (année type)
# Usage :  python simulation_chanvre.py H3          ou      python simulation_chanvre.py H1c Th-BC
ZONE = sys.argv[1] if len(sys.argv) > 1 else "H1c"
ZONE = "MACON" if ZONE.upper() == "MACON" else ZONE[:2].upper() + ZONE[2:].lower()   # h1C → H1c
MODE = sys.argv[2] if len(sys.argv) > 2 else "Th-D"
...
# ══════════════════════════════════════════════════════════════════════════════
# 3B. GRAPHIQUE — Modèle 3D de la pièce
# ══════════════════════════════════════════════════════════════════════════════
...
fig_3d = plt.figure(figsize=(13, 9))
ax3d = fig_3d.add_subplot(111, projection="3d")
ax3d.set_facecolor("#F8F8F8")

# ── Parois (boîtes 3D jointives aux coins) ────────────────────────────────────
# Sud/Nord couvrent toute la largeur extérieure (y compris les coins),
# Est/Ouest remplissent entre elles.
walls_3d = [
...
for nom, x0, x1, y0, y1, col, lpos in walls_3d:
    poly = Poly3DCollection(_box_faces(x0,x1,y0,y1,0,H),
...
w_s_w, w_s_h = 1.2, 1.25     # Vitrage Sud : 1.5 m²
w_s_x, w_s_z = (Lx-w_s_w)/2, 0.90
...
w_n_w, w_n_h = 0.70, 0.71    # Vitrage Nord : 0.5 m²
w_n_x, w_n_z = (Lx-w_n_w)/2, 1.20
...
try:    fig_3d.savefig(_out, dpi=150, bbox_inches="tight"); print("  [OK] modele_3d.pdf")
except PermissionError:
    fig_3d.savefig(_out.replace(".pdf","_new.pdf"), dpi=150, bbox_inches="tight")
plt.close(fig_3d)
...
# ══════════════════════════════════════════════════════════════════════════════
# 5. GRAPHIQUE 1 — Série temporelle T et HR  (pièce vs extérieur)
# ══════════════════════════════════════════════════════════════════════════════

print("\n  Tracé : serie_temp_HR.pdf ...")
...
T_moy_p  = np.array([T_arr[d*steps_j:(d+1)*steps_j].mean()    for d in range(n_j)])
...
fig5, (ax5T, ax5H) = plt.subplots(2, 1, figsize=(14, 9), constrained_layout=True)
fig5.suptitle(
    f"Béton de chanvre {EPAISSEUR_M*100:.0f} cm — Pièce {Lx:.0f}×{Ly:.0f} m  "
    f"| {LABEL_METEO} | 21 juin → 21 septembre\n"
    f"Surventilation nocturne {ACH_NUIT} vol/h (RE2020)  |  "
    f"Hygroscopique : {'actif' if EFFET_HYGRO else 'désactivé'}",
    fontsize=11, fontweight="bold"
)

# Température
ax5T.fill_between(jours, T_min_e, T_max_e, color="#DDDDDD", alpha=0.55,
                  label="Plage extérieure (min–max)")
ax5T.fill_between(jours, T_min_p, T_max_p, color="#E07020", alpha=0.22,
                  label="Plage pièce (min–max)")
ax5T.plot(jours, T_moy_e, color="#888888", lw=1.5, ls="--", label="T moy. extérieure")
ax5T.plot(jours, T_moy_p, color="#CC5500", lw=2.2, label="T moy. pièce")
ax5T.axhline(28, color="#CC3030", lw=1.0, ls=":", alpha=0.75,
             label="Seuil RE2020 DH = 28 °C")
ax5T.set_ylabel("Température [°C]", fontsize=11)
ax5T.set_xlim(0, n_j-1)
if tick_j: ax5T.set_xticks(tick_j); ax5T.set_xticklabels(tick_lb, fontsize=10)
ax5T.legend(fontsize=10, loc="upper right", framealpha=0.92)
ax5T.grid(True, alpha=0.18)
ax5T.text(0.01, 0.97,
    f"T max pièce : {T_arr.max():.1f} °C   T max ext : {T_ext_f.max():.1f} °C\n"
    f"Amortissement : ×{T_arr.max()/T_ext_f.max():.2f}  |  DH = {DH_total:.0f} °C·h",
    transform=ax5T.transAxes, fontsize=9.5, va="top",
    bbox=dict(boxstyle="round,pad=0.4", fc="white", ec="#BBBBBB", alpha=0.92))

# HR
ax5H.plot(jours, HR_moy_e, color="#AAAAAA", lw=1.5, ls="--", label="HR moy. extérieure")
ax5H.plot(jours, HR_moy_p, color="#334499", lw=2.2, label="HR moy. pièce")
ax5H.axhline(30, color="#E07020", lw=0.8, ls=":", alpha=0.6, label="Limite basse confort 30%")
ax5H.axhline(70, color="#CC3030", lw=0.8, ls=":", alpha=0.6, label="Limite haute confort 70%")
ax5H.set_ylim(0, 105)
ax5H.set_ylabel("Humidité relative [%]", fontsize=11)
ax5H.set_xlabel("Période simulée", fontsize=11)
ax5H.set_xlim(0, n_j-1)
if tick_j: ax5H.set_xticks(tick_j); ax5H.set_xticklabels(tick_lb, fontsize=10)
ax5H.legend(fontsize=10, loc="upper right", framealpha=0.92)
ax5H.grid(True, alpha=0.18)

_out = _pdf("serie_temp_HR")
try:    fig5.savefig(_out, dpi=150, bbox_inches="tight"); print(f"  [OK] {os.path.basename(_out)}")
except PermissionError:
...
_out = _pdf("profils_parois_NS")
try:    fig6.savefig(_out, dpi=150); print(f"  [OK] {os.path.basename(_out)}")
except PermissionError:
...
fig7.suptitle(
    f"Données climatiques utilisées — {LABEL_METEO}\n"
...
_out = _pdf("climat")
try:    fig7.savefig(_out, dpi=150); print(f"  [OK] {os.path.basename(_out)}")
except PermissionError:
...
_out = _pdf("confort_map")
try:    fig8.savefig(_out, dpi=150); print(f"  [OK] {os.path.basename(_out)}")
except PermissionError:
...
fig9.suptitle(
    ...
    f"DH total = {DH_total:.0f} °C·h  ({DH_total/1250*100:.0f}% du seuil RE2020 = 1 250 °C·h/an)",
    fontsize=12, fontweight="bold"
)
...
_out = _pdf("degres_heures")
try:    fig9.savefig(_out, dpi=150); print(f"  [OK] {os.path.basename(_out)}")
except PermissionError:
...
print("  FICHIERS")
print("  modele_3d.pdf")
for f in ["serie_temp_HR","profils_parois_NS","climat","confort_map","degres_heures"]:
    print(f"  {f}{SUF}.pdf")
print("="*60)
